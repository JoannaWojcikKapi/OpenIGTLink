...
int main(int argc, char* argv[])
{
	...
	while (true)
	{
		igtl::MessageHeader::Pointer messageHeader;
		messageHeader = igtl::MessageHeader::New();/*
			int r1 = ReceivePoint(socket, messageHeader);*/
			...
			for (int i = 0; i <= 3; i++){
				...
				messageHeader->InitPack();

				// Receive generic header from the socket
				int r = socket->Receive(messageHeader->GetPackPointer(), messageHeader->GetPackSize());
				if (r == 0)
				{
					socket->CloseSocket();
				}
				if (r != messageHeader->GetPackSize())
				{
					continue;
				}
				...
				if (strcmp(messageHeader->GetDeviceType(), "POINT") == 0)
				{
					ReceivePoint(socket, messageHeader);
				}
				else
				{
					std::cerr << "Unvalid format of header message" << std::endl;
					std::cerr << "Receiving : " << messageHeader->GetDeviceType() << std::endl;
					std::cerr << "Size : " << messageHeader->GetBodySizeToRead() << std::endl;
					socket->Skip(messageHeader->GetBodySizeToRead(), 0);
				}
			}
	}
