...
igtl::PointElement::Pointer ModifyPointer(igtl::PointElement::Pointer pointer);
...
#if OpenIGTLink_PROTOCOL_VERSION >= 2
igtl::PointMessage::Pointer ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header);
...
int main(int argc, char* argv[])
{
	...
	igtl::PointMessage::Pointer receiverdPoint;
	bool serviceReceivedData = false;
	while (!serviceReceivedData)
	{
		//------------------------------------------------------------
		...
		if (socket.IsNotNull()) // if client connected
		{
			...
			{
				receiverdPoint = ReceivePoint(socket, headerMsg);
				serviceReceivedData = true;
			}
			else
			{
	...
	//Receiving point from client and sending its to client
	igtl::PointElement::Pointer newPoint;
	int nElements = receiverdPoint->GetNumberOfPointElement();
	for (int i = 0; i < nElements; i++)
	{
		igtl::PointElement::Pointer pointElement;
		receiverdPoint->GetPointElement(i, pointElement);
		newPoint = ModifyPointer(pointElement);
		igtl::Sleep(100);
		igtl::PointMessage::Pointer pointMsg;
		pointMsg = igtl::PointMessage::New();
		pointMsg->SetDeviceName("PointSender");
		pointMsg->AddPointElement(newPoint);
		pointMsg->Pack();
		int t = socket->Send(pointMsg->GetPackPointer(), pointMsg->GetPackSize());
		std::cerr << "Point sended." << std::endl;
	}
...
igtl::PointElement::Pointer ModifyPointer(igtl::PointElement::Pointer pointer){
	// Changing points coordinate marks
	igtlFloat32 pos[3];
	pointer->GetPosition(pos);
	pos[0] = 0 - pos[0];
	pos[1] = 0 - pos[1];
	pos[2] = 0 - pos[2];
